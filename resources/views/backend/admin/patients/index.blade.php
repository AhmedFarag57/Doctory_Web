...
@section('content')
    ...
    <div class="roles-permissions">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold">Patients</h2>
            </div>
            <div class="flex flex-wrap items-center">
                <a href="{{ route('patients.create') }}" class="bg-gray-200 text-gray-700 text-sm uppercase py-2 px-4 flex items-center rounded">
                    <svg class="w-3 h-3 fill-current" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="plus" class="svg-inline--fa fa-plus fa-w-14" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path fill="currentColor" d="M416 208H272V64c0-17.67-14.33-32-32-32h-32c-17.67 0-32 14.33-32 32v144H32c-17.67 0-32 14.33-32 32v32c0 17.67 14.33 32 32 32h144v144c0 17.67 14.33 32 32 32h32c17.67 0 32-14.33 32-32V304h144c17.67 0 32-14.33 32-32v-32c0-17.67-14.33-32-32-32z"></path></svg>
                    <span class="ml-2 text-xs font-semibold">Patient</span>
                </a>
            </div>
        </div>
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            ...
            <div class="flex flex-wrap items-center uppercase text-sm font-semibold bg-gray-300 text-gray-600 rounded-tl rounded-tr">
                ...
                <div class="w-2/12 px-4 py-3">Wallet</div>
                <div class="w-2/12 px-4 py-3">Blocked</div>
            ...
            @foreach ($patients as $patient)
                ...
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    ...
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $patient->user->name }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $patient->user->email }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $patient->wallet }}</div>
                    ...
                    <div class="w-2/12 flex items-center justify-end">
                        <a class="px-2" href="{{ route('patients.show', $patient->id) }}">
                        ...
                        <a href="{{ route('patients.destroy', $patient->id) }}" data-url="{{ route('patients.destroy', $patient->id) }}" class="deletebtn ml-1 bg-gray-600 block p-1 border border-gray-600 rounded-sm">
                        ...
                        <a class="px-1" href="{{ route('users.block', $patient->user->id) }}">
                            <svg class="h-6 w-6 fill-current text-gray-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6"><path fill-rule="evenodd" d="M6.72 5.66l11.62 11.62A8.25 8.25 0 006.72 5.66zm10.56 12.68L5.66 6.72a8.25 8.25 0 0011.62 11.62zM5.105 5.106c3.807-3.808 9.98-3.808 13.788 0 3.808 3.807 3.808 9.98 0 13.788-3.807 3.808-9.98 3.808-13.788 0-3.808-3.807-3.808-9.98 0-13.788z" clip-rule="evenodd" /></svg>                              
                        </a>
                    </div>
        ...
        @include('backend.modals.delete', ['name' => 'patient'])
